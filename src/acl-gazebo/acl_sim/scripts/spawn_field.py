...
class fieldMaster:
	...
	def checkForGoal(self):
		if np.linalg.norm(self.pose-self.goal) < 1:
	...
	def start_up(self):
		rospy.logwarn("Starting simulation # %i of %i",self.count+1,self.N)
	...
	def kill_process(self):
		...
		rospy.logfatal("Processes killed")
	...
	def start_process(self):
		...
		os.system("roslaunch acl_planning tip.launch bag:=0 quad:=LQ02s &")


	def openFile(self):
		self.tfile = open("/home/brett/logs/test_log_" + time.strftime("%H-%M-%S_%d-%m-%Y") + "_mem_"+str(self.memory)+"_lam_"+str(self.lam)+".txt",'w')

	def saveFile(self):
		if self.first:
			self.tfile.write("Iteration Result Obstacles \n")  
			self.first = False
		self.tfile.write("%i %i %i %0.4f  \n" % (self.count+1, self.status, self.rho, self.t_goal))
