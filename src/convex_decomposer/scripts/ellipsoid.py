...
def dist_pt_ellipsoid_centre(pt,Cinv,d):
  distance = norm(Cinv.dot(pt-d))
  return distance

def pt_is_inside_ep(pt,Cinv,d):
    if norm(Cinv.dot(pt-d))<=1:
      return True
    return False
...
def find_ellipsoid(p1,p2,offset_x,obs,inflate_distance):
    ...
    while (obs_inside !=[]):
        ...
        obs_inside = obs_retain
    return C,d,Cinv,obs#use the last return -> a list that can bve returned to display obstacles on xnewcloud publisher, for debug only