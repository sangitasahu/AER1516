...
inline bool line_intersect(const std::pair<Vec2f, Vec2f> &v1,
  ...
  //std::cout<<"abcabc:"<<a1<<" "<<b1<<" "<<c1<<" "<<a2<<" "<<b2<<" "<<c2<<" "<<std::endl;
  ...
  for (unsigned int i = 0; i < lines.size(); i++) {
    for (unsigned int j = i + 1; j < lines.size(); j++) {
      ...
      if (line_intersect(lines[i], lines[j], pi)) {
        //std::cout<<"Comparing line1: ["<<lines[i].first(0)<<" "<<lines[i].first(1)<<" "<<lines[i].second(0)<<" "<<lines[i].second(1)<<"] line2: ["<<lines[j].first(0)<<" "<<lines[j].first(1)<<" "<<lines[j].second(0)<<" "<<lines[j].second(1)<<"]"<<std::endl;
        pts.push_back(pi);
        //std::cout<<"pt:"<<pi(0)<<" "<<pi(1)<<"line1: ["<<lines[i].first(0)<<" "<<lines[i].first(1)<<" "<<lines[i].second(0)<<" "<<lines[i].second(1)<<"] line2: ["<<lines[j].first(0)<<" "<<lines[j].first(1)<<" "<<lines[j].second(0)<<" "<<lines[j].second(1)<<"]"<<std::endl;
      }
    }
  }
  return pts;
