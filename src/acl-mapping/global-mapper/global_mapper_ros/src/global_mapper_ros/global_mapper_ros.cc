...
#include <sensor_msgs/PointCloud.h> //Vandan Added this
#include <sensor_msgs/point_cloud_conversion.h>//Vandan Added this
...
void GlobalMapperRos::InitPublishers()
{
  if (publish_occupancy_grid_)
  {
    ...
    occ_grid1_pub_ = pnh_.advertise<sensor_msgs::PointCloud>("occupancy_grid1_topic", 1); //Vandan added this
  }
...
void GlobalMapperRos::Publish(const ros::TimerEvent& event)
{
  ...
  if (publish_occupancy_grid_)
  {
    ...
    sensor_msgs::PointCloud occupancy_info_msg;
    occupancy_info_msg.header.frame_id = "vicon";
    occupancy_info_msg.header.stamp = ros::Time(0);   
    sensor_msgs::convertPointCloud2ToPointCloud(occ_pointcloud_msg,occupancy_info_msg); //Vandan Added this
    occ_grid_pub_.publish(occ_pointcloud_msg);
    occ_grid1_pub_.publish(occupancy_info_msg); //Vandan Added this
  }
